...
export const viewUserProfile = async (userId: string): Promise<AuthUser> => {
    ...
    // Optional: Update localStorage so the local backup stays in sync
    persistUserProfile(data);

    return data;
};

export const updateUserProfile = async (user: AuthUser): Promise<AuthUser> => {
    const response = await fetch(`${AUTH_API_BASE_URL}/user/update`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            userId: user.id, 
    ...
    const data = await parseApiResponse<AuthUser & { message?: string }>(
        response,
        "The server returned an unexpected response while updating your profile."
    );
    ...
    // After a successful server update, sync it locally
